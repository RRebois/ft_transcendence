...
class MatchMaking():
    ...
    def add_player(session_id, username):
        logging.debug(f"add_player: {username}")
        if MatchMaking.matchs.get(session_id):
            if username not in ['guest', 'bot']:
                try:
                    user_data = UserData.objects.get(user_id=User.objects.get(username=username))
                    elo = user_data.user_elo_pong[-1] if MatchMaking.matchs[session_id]['game_name'] == 'pong' else \
                    user_data.user_elo_purrinha[-1]
    ...
    def find_match(user_elo, game_name, game_code, diff):
        ...
        for k, v in MatchMaking.matchs.items():
            if v['status'] == 'open' and v['players'] and not v['tournament_id'] \
                ...
                matchs_open += 1
                # elo_diff = [abs(elo - user_elo['elo']) for elo in v['players'] if abs(elo - user_elo['elo']) <= diff]
                # logging.debug("------------------------------------------ FOR LOOP ------------------------------------------")
                for player in v['players']:
                    if 'elo' in player:
                        logging.debug(f"player[elo]: {player['elo']}")
                        logging.debug(f"user_elo: {user_elo['elo']}")
                        comparison = abs(player['elo'] - user_elo['elo'])
                        logging.debug(f"Comparing user_elo {user_elo} with player_elo {player['elo']}: {comparison}")
                        if comparison <= diff:
                            if elo_diff is None:
                                elo_diff = []
                            elo_diff.append(comparison)
                if elo_diff:
                    return k
        if matchs_open == 0:
            return None
...
class GameManagerView(APIView):
    ...
    def get(self, request, game_name, game_code):
        ...
        logging.debug(f"GameManagerView.get: {username} got session_id {session_id} ==> send response 200")
        return JsonResponse({
            'game': game_name,
            'session_id': session_id,
